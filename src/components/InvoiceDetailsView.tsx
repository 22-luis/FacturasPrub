...
import { BadgeDollarSign, CalendarDays, Hash, Building, Fingerprint, FileText, MapPin } from 'lucide-react';
import type { ExtractedInvoiceDetails, AssignedInvoice } from '@/lib/types';

type InvoiceData = Partial<ExtractedInvoiceDetails & Pick<AssignedInvoice, 'uniqueCode' | 'address'>>;
...
const iconMap: Record<keyof InvoiceData, React.ReactNode | undefined> = {
  ...
  address: <MapPin className="h-5 w-5 text-muted-foreground" />, // Added address icon
};

const labelMap: Record<keyof InvoiceData, string | undefined> = {
  invoiceNumber: 'Invoice Number',
  date: 'Date',
  totalAmount: 'Total Amount',
  supplierName: 'Supplier Name',
  uniqueCode: 'Unique Code',
  address: 'Dirección', // Added address label
};
        ...
        {entries.length === 0 ? (
          ...
          <ul className="space-y-3">
            {(Object.keys(labelMap) as Array<keyof InvoiceData>).map((key) => {
              ...
              if (value === undefined || value === null || value === '' || !label || !icon) return null;
